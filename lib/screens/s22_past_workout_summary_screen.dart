...
class S22PastWorkoutSummaryScreen extends StatelessWidget {
  final WorkoutDay? workoutDay;
  final DateTime? targetDate;

  const S22PastWorkoutSummaryScreen({Key? key, this.workoutDay, this.targetDate}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    if (workoutDay == null || targetDate == null) {
      return const Scaffold(
        backgroundColor: AppColors.background,
        body: Center(child: Text('Error: Missing data', style: AppTextStyles.bodyMedium)),
      );
    }
    ...
    Map<dynamic, dynamic>? mostRecentLog;
    DateTime? mostRecentTimestamp;

    for (var key in box.keys) {
      final entry = box.get(key);
      if (entry is Map && entry['date'] == targetDateStr) {
    ...
    // Check if legacy log (missing new fields)
    bool hasValidData = mostRecentLog != null && 
        mostRecentLog['durationSeconds'] != null &&
        mostRecentLog['totalVolume'] != null &&
        mostRecentLog['completedSets'] != null;
    ...
    if (!hasValidData) {
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text('PAST WORKOUT', style: AppTextStyles.labelAllcaps),
          centerTitle: true,
        ),
        body: const Center(
          child: Text('No workout completed yet for this day.', style: AppTextStyles.bodyMedium),
        ),
      );
    }

    // Extract exactly matching the write keys
    int duration = mostRecentLog!['durationSeconds'] as int;
    double volume = (mostRecentLog!['totalVolume'] as num).toDouble();
    int completedSets = mostRecentLog!['completedSets'] as int;
    List<String> completedExerciseNames = List<String>.from(mostRecentLog!['completedExerciseNames'] ?? []);
    ...
    String setsStr = completedSets.toString();
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      mostRecentLog!['workoutName'] ?? workoutDay!.workoutName,
                    ...
                    _buildMusclesWorkedGrid(completedExerciseNames),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildMusclesWorkedGrid(List<String> completedExerciseNames) {
